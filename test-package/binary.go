...
func parseVarInstance(data []byte, coordsLength int, coords2Length int) (VarInstance, error) {
	var out VarInstance
	n := 0
	tmp0 := data[n:]
	if L := len(tmp0); L < 0+coordsLength*4 {
		return VarInstance{}, fmt.Errorf("EOF: expected length: %d, got %d", 0+coordsLength*4, L)
	}
	out.Coords = make([]fl1616, coordsLength)
	for i := range out.Coords {
		chunk := tmp0[0+i*4:]
		out.Coords[i] = fl1616FromUint(binary.BigEndian.Uint32(chunk[0:4]))

	}
	n += 0 + coordsLength*4

	tmp1 := data[n:]
	if L := len(tmp1); L < 0+coords2Length*4 {
		return VarInstance{}, fmt.Errorf("EOF: expected length: %d, got %d", 0+coords2Length*4, L)
	}
	out.Coords2 = make([]fl1616, coords2Length)
	for i := range out.Coords2 {
		chunk := tmp1[0+i*4:]
		out.Coords2[i] = fl1616FromUint(binary.BigEndian.Uint32(chunk[0:4]))
	...
	tmp2 := data[n:]
	if L := len(tmp2); L < 4 {
		return VarInstance{}, fmt.Errorf("EOF: expected length: 4, got %d", L)
	}
	_ = tmp2[3] // early bound checking
	out.Subfamily = uint16(binary.BigEndian.Uint16(tmp2[0:2]))
	out.PSStringID = uint16(binary.BigEndian.Uint16(tmp2[2:4]))
	n += 4

	return out, nil
}

func (item VarInstance) appendTo(data []byte) []byte {
	L0 := len(data)
	data = append(data, make([]byte, 0+len(item.Coords)*4)...)
	dst0 := data[L0:]
	for i, v := range item.Coords {
		chunk := dst0[0+i*4:]
		binary.BigEndian.PutUint32(chunk, uint32(fl1616ToUint(v)))
	}
	L1 := len(data)
	data = append(data, make([]byte, 0+len(item.Coords2)*4)...)
	dst1 := data[L1:]
	for i, v := range item.Coords2 {
		chunk := dst1[0+i*4:]
		binary.BigEndian.PutUint32(chunk, uint32(fl1616ToUint(v)))
	}
	L2 := len(data)
	...
	dst2 := data[L2:]
	_ = dst2[3] // early bound checking
	binary.BigEndian.PutUint16(dst2, uint16(item.Subfamily))
	binary.BigEndian.PutUint16(dst2[2:], uint16(item.PSStringID))
...
func parseComplexeSubtable(data []byte) (complexeSubtable, error) {
	...
	if L := len(tmp4); L < 8+arrayLength4*4 {
		return complexeSubtable{}, fmt.Errorf("EOF: expected length: %d, got %d", 8+arrayLength4*4, L)
	}
	out.array3 = make([]fl32, arrayLength4)
	for i := range out.array3 {
		chunk := tmp4[8+i*4:]
		out.array3[i] = fl32FromUint(binary.BigEndian.Uint32(chunk[0:4]))
...
func (item complexeSubtable) appendTo(data []byte) []byte {
	...
	L4 := len(data)
	data = append(data, make([]byte, 8+len(item.array3)*4)...)
	dst4 := data[L4:]
	binary.BigEndian.PutUint64(dst4, uint64(len(item.array3)))
	for i, v := range item.array3 {
		chunk := dst4[8+i*4:]
		binary.BigEndian.PutUint32(chunk, uint32(fl32ToUint(v)))
	}
...
func (out *lookup) mustParse(data []byte) {
	...
	out.v.fromUint(binary.BigEndian.Uint16(data[30:32]))
	out.w = fl32FromUint(binary.BigEndian.Uint32(data[32:36]))
}
...
func (item lookup) writeTo(data []byte) {
	...
	binary.BigEndian.PutUint16(data[30:], uint16(item.v.toUint()))
	binary.BigEndian.PutUint32(data[32:], uint32(fl32ToUint(item.w)))
}
